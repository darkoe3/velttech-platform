      ...
      {questions.map((question, index) => (
        <div key={index} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <textarea
            ...
            className="mt-3 min-h-16 w-full rounded-lg border border-slate-300 px-4 py-3"
          />
        </div>
      ))}
      <button
...
export function AssessmentScoreTables({ results = [] }) {
  if (!results.length) {
    return null;
  }
  ...
  return (
    <div className="space-y-6">
      <ScoreEntrySection
        ...
        endpoint="/api/instructor/assessment-results/practical"
        scoreField="practical_score"
        maxField="practical_max_score"
        results={results}
      />
      <ScoreEntrySection
        title="Final Project Scores"
        endpoint="/api/instructor/assessment-results/final-project"
        scoreField="final_project_score"
        maxField="final_project_max_score"
        feedbackField="final_project_feedback"
        results={results}
      />
      <CombinedResultsTable results={results} />
    </div>
  );
}

function ScoreEntrySection({ title, endpoint, scoreField, maxField, feedbackField, results }) {
  return (
...
function AssessmentScoreForm({ endpoint, result, scoreField, maxField, feedbackField }) {
  const router = useRouter();
  const [error, setError] = useState("");
  const [pending, setPending] = useState(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg bg-white p-4">
      <div className={`grid gap-3 ${feedbackField ? "lg:grid-cols-[1fr_0.25fr_0.25fr_1fr_auto]" : "lg:grid-cols-[1fr_0.25fr_0.25fr_auto]"}`}>
        <div>
          <p className="font-semibold text-dark">{result.student_name}</p>
          <p className="text-sm text-slate-500">{result.course_title}</p>
        </div>
        <input name="score" type="number" min="0" step="0.01" max={result[maxField]} defaultValue={result[scoreField] ?? ""} placeholder="Score" className="rounded-lg border border-slate-300 px-4 py-3" />
        <input name="max_score" type="number" min="1" defaultValue={result[maxField]} className="rounded-lg border border-slate-300 px-4 py-3" />
        {feedbackField ? (
          <input name="feedback" defaultValue={result[feedbackField] || ""} placeholder="Feedback" className="rounded-lg border border-slate-300 px-4 py-3" />
        ) : null}
        <button disabled={pending} className="rounded-lg bg-dark px-4 py-3 text-sm font-bold text-white disabled:opacity-60">
          {pending ? "Saving..." : "Save"}
        </button>
...
function CombinedResultsTable({ results }) {
  return (
    <div className="overflow-x-auto rounded-lg border border-slate-200">
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody className="divide-y divide-slate-100 bg-white">
          {results.map((result) => (
            <tr key={`combined-${result.id}`}>
              <td className="px-4 py-3 font-semibold text-dark">{result.student_name}</td>
              <td className="px-4 py-3">{result.practical_score ?? "-"} / {result.practical_max_score}</td>
              <td className="px-4 py-3">{result.final_project_score ?? "-"} / {result.final_project_max_score}</td>
              <td className="px-4 py-3">{result.objective_quiz_score ?? "-"} / {result.objective_quiz_max_score}</td>
              <td className="px-4 py-3 font-bold">{result.overall_score} / {result.total_max_score} ({result.percentage}%)</td>
              <td className="px-4 py-3">{result.final_project_feedback || "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export function ParentChildAssessments({ childrenAssessments = [] }) {
  if (!childrenAssessments.length) return null;

  return (
    <section className="mt-8">
      <div className="space-y-6">
        {childrenAssessments.map((child) => (
          <div key={child.student} className="rounded-lg border border-slate-200 bg-white p-5">
            <h2 className="text-lg font-bold text-dark">{child.student_name}</h2>
            <div className="mt-4 grid gap-5 lg:grid-cols-2">
              <div className="space-y-4">
                <h3 className="font-bold text-dark">Available Quizzes</h3>
                {child.quizzes.length === 0 ? (
                  <p className="rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">No objective quiz available.</p>
                ) : (
                  child.quizzes.map((quiz) => (
                    <ChildQuizForm key={quiz.id} childId={child.student} quiz={quiz} />
                  ))
                )}
              </div>
              <div className="space-y-4">
                <h3 className="font-bold text-dark">Combined Results</h3>
                {child.results.length === 0 ? (
                  <p className="rounded-lg bg-slate-50 px-4 py-3 text-sm text-slate-600">No scores recorded yet.</p>
                ) : (
                  child.results.map((result) => (
                    <div key={result.id} className="rounded-lg bg-slate-50 p-4 text-sm">
                      <p className="font-bold text-dark">{result.course_title}</p>
                      <dl className="mt-3 grid gap-2 sm:grid-cols-2">
                        <div><dt className="text-slate-500">Practical</dt><dd className="font-bold">{result.practical_score ?? "-"} / {result.practical_max_score}</dd></div>
                        <div><dt className="text-slate-500">Final Project</dt><dd className="font-bold">{result.final_project_score ?? "-"} / {result.final_project_max_score}</dd></div>
                        <div><dt className="text-slate-500">Objective Quiz</dt><dd className="font-bold">{result.objective_quiz_score ?? "-"} / {result.objective_quiz_max_score}</dd></div>
                        <div><dt className="text-slate-500">Overall</dt><dd className="font-bold">{result.overall_score} / {result.total_max_score} ({result.percentage}%)</dd></div>
                      </dl>
                      {result.final_project_feedback ? <p className="mt-3 text-slate-600">{result.final_project_feedback}</p> : null}
                    </div>
                  ))
                )}
              </div>
...
function ChildQuizForm({ childId, quiz }) {
  const router = useRouter();
  ...
  async function handleSubmit(event) {
    ...
    setError("");
    try {
      await requestJson(`/api/my-children-assessments/${childId}/${quiz.id}/submit`, "POST", { answers });
      router.refresh();
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="font-bold text-dark">{quiz.title}</p>
          <p className="mt-1 text-sm text-slate-500">{quiz.course_title}</p>
        </div>
        {!locked && !started ? (
          <button type="button" onClick={() => setStarted(true)} className="rounded-lg bg-dark px-4 py-2 text-sm font-bold text-white">
            Start Quiz
          </button>
        ) : null}
      </div>
      ...
      {locked ? (
        <p className="mt-3 rounded-lg bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">
          Submitted: {resultLabel(existingSubmission, quiz.marks)}
        </p>
      ) : null}
      {started && !locked ? (
        <div className="mt-4 space-y-3">
          {questions.map((question, index) => (
            <fieldset key={question.id} className="rounded-lg bg-white p-4">
              <legend className="font-bold text-dark">Question {index + 1}</legend>
              <p className="mt-2 text-sm text-slate-700">{question.question_text}</p>
              <div className="mt-3 grid gap-2">
                {[
                  ...
                  <label key={value} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm">
                    <input
                      type="radio"
                      name={`child-${childId}-question-${question.id}`}
                      value={value}
                      checked={answers[String(question.id)] === value}
                      required
                      onChange={() => setAnswers((current) => ({ ...current, [String(question.id)]: value }))}
          ...
          <button disabled={pending || !questions.length} className="rounded-lg bg-dark px-4 py-3 text-sm font-bold text-white disabled:opacity-60">
            {pending ? "Submitting..." : "Submit Quiz"}
          </button>
        </div>
      ) : null}
