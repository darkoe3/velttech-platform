...
function AssessmentResultSummary({ results = [] }) {
  if (!results.length) {
    return <EmptyState>No combined assessment results yet.</EmptyState>;
  }

  return (
    <div className="space-y-3">
      {results.map((result) => (
        <div key={result.id} className="rounded-xl bg-slate-50 px-4 py-3 text-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="font-bold text-dark">{result.course_title}</p>
              <p className="mt-1 text-slate-500">Overall {result.overall_score} / {result.total_max_score}</p>
            </div>
          ...
          <dl className="mt-3 grid gap-2 sm:grid-cols-3">
            <div>
              <dt className="font-semibold text-slate-500">Practical</dt>
              <dd className="font-bold text-dark">{result.practical_score ?? "-"} / {result.practical_max_score}</dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-500">Final Project</dt>
              <dd className="font-bold text-dark">{result.final_project_score ?? "-"} / {result.final_project_max_score}</dd>
            </div>
            <div>
              <dt className="font-semibold text-slate-500">Objective Quiz</dt>
              <dd className="font-bold text-dark">{result.objective_quiz_score ?? "-"} / {result.objective_quiz_max_score}</dd>
            </div>
          </dl>
...
function StudentDashboard({ dashboard }) {
  ...
  const assessmentResults = dashboard.assessment_results || [];
    ...
    <>
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <section>
          ...
          <AcademyCard className="mt-5">
            <h3 className="mb-4 font-bold text-dark">Combined Scores</h3>
            <AssessmentResultSummary results={assessmentResults} />
          </AcademyCard>
        </section>
              ...
              {children.map((child) => (
                <AcademyCard key={child.id}>
                  ...
                  <div className="mt-5 border-t border-slate-100 pt-5">
                    <h4 className="mb-3 text-sm font-bold text-dark">Assessment Results</h4>
                    <AssessmentResultSummary results={child.assessment_results || []} />
                  </div>
                  <div className="mt-5 border-t border-slate-100 pt-5">
