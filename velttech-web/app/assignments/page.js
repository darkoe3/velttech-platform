import { AssignmentForm, InstructorAssignmentActions, ParentChildAssessments, SubmissionForm } from "@/components/assignments/AssignmentForms";
...
export default async function AssignmentsPage() {
  try {
    ...
    const [assignments, courses, enrollments, dashboard, childrenAssessments] = await Promise.all([
      ...
      user.role === "parent" ? djangoApiFetch("my-children-assessments") : Promise.resolve([]),
    ]);
    return (
      <section className="mx-auto max-w-7xl px-5 py-10">
        ...
        {user.role === "parent" ? <ParentChildAssessments childrenAssessments={childrenAssessments} /> : null}
      </section>
    );
  } catch (error) {
